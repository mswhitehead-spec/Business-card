...
function setup() {
  ...
  if (!hasIt) {
    ScriptApp.newTrigger('processNewCards').timeBased().everyMinutes(5).create();
  }
...
function processNewCards() {
  ...
  if (!folderId || !sheetId) { Logger.log('Run setup() first.'); return; }
...
function parseContact(text) {
  ...
  var phone = (text.match(/(?:\+?[\d]{1,3}[\s.\-]?)?\(?[\d]{3}\)?[\s.\-]?[\d]{3}[\s.\-]?[\d]{4}(?:\s?(?:x|ext)\.?\s?[\d]{1,5})?/) || [''])[0].trim();
  ...
  var addrLine = '', addrIdx = -1;
  for (var i = 0; i < lines.length; i++) {
    if (/\d+\s+\w/.test(lines[i]) && /(?:st|ave|blvd|rd|dr|ln|way|court|pl|suite|ste|floor)/i.test(lines[i])) {
      addrLine = lines[i]; addrIdx = i; break;
    }
  }
  var address = addrLine ? addrLine + (addrIdx >= 0 && lines[addrIdx + 1] ? ', ' + lines[addrIdx + 1] : '') : '';
  ...
  var textLines = lines.filter(function(l) {
    ...
    return !dataPatterns.some(function(p) { return l.toLowerCase().indexOf(p.replace('https://', '').replace('@', '').toLowerCase()) !== -1; });
  });
  function looksLikeName(line) {
    var words = line.trim().split(/\s+/);
    return words.length >= 2 && words.length <= 5 && words.every(function(w) { return /^[A-Z][a-zA-Z'\-]{0,}$/.test(w); });
  }
  ...
  var nameLine = '';
  for (var j = 0; j < textLines.length; j++) { if (looksLikeName(textLines[j])) { nameLine = textLines[j]; break; } }
  if (!nameLine && textLines.length > 0) nameLine = textLines[0];
  var rest = textLines.filter(function(l) { return l !== nameLine; });
  var titleLine = '';
  for (var k = 0; k < rest.length; k++) { if (TITLE_RE.test(rest[k])) { titleLine = rest[k]; break; } }
  if (!titleLine && rest.length > 0) titleLine = rest[0];
  var rest2 = rest.filter(function(l) { return l !== titleLine; });
  var companyLine = '';
  for (var m = 0; m < rest2.length; m++) { if (COMPANY_RE.test(rest2[m])) { companyLine = rest2[m]; break; } }
  if (!companyLine && rest2.length > 0) companyLine = rest2[0];
  return { name: nameLine, title: titleLine, company: companyLine, email: email, phone: phone, website: website, address: address, linkedIn: linkedIn, twitter: twitter, notes: '' };
}

function addToSheet(sheet, contact, imageUrl) {
  sheet.appendRow([contact.name, contact.title, contact.company, contact.email, contact.phone, contact.website, contact.address, contact.linkedIn, contact.twitter, contact.notes, imageUrl, new Date().toLocaleString()]);
}
