...
export function ContactCard({ contact, onClick }: Props) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left bg-white rounded-xl border border-gray-200 p-4 shadow-sm hover:shadow-md active:scale-[0.98] transition-all flex flex-col gap-3"
    >
      <div className="flex items-center gap-3">
        {contact.imageDataUrl ? (
          <img
            ...
            alt={contact.name}
            className="w-12 h-8 object-cover rounded-md border border-gray-100 flex-shrink-0"
          />
        ) : (
          <div className={`w-10 h-10 rounded-full ${color} flex items-center justify-center text-white font-semibold text-lg flex-shrink-0`}>
            {initial}
          </div>
        )}
