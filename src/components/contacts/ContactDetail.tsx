...
export function ContactDetail({ contactId }: Props) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <header className="sticky top-0 bg-white z-10 px-4 pt-4 pb-3 border-b border-gray-200 flex items-center gap-3">
        <button
          onClick={() => dispatch({ type: 'SET_VIEW', view: { name: 'list' } })}
          className="flex items-center text-blue-600 font-medium text-sm gap-1"
        >
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <ContactField
            ...
            href={contact.linkedIn ? (contact.linkedIn.startsWith('http') ? contact.linkedIn : `https://linkedin.com/in/${contact.linkedIn}`) : undefined}
          />
          <ContactField
            icon="twitter"
            label="Twitter / X"
            value={contact.twitter}
            href={contact.twitter ? `https://x.com/${contact.twitter.replace('@', '')}` : undefined}
          />
        </div>
        ...
        {showDeleteConfirm ? (
          ...
          <button
            ...
            className="w-full py-3 text-red-600 font-medium text-sm border border-red-200 rounded-xl hover:bg-red-50 transition-colors"
          >
            Delete Contact
          </button>
        )}
...
function ContactField({ icon, label, value, href }: FieldProps) {
  ...
  const content = (
    <div className="flex items-start gap-3 px-4 py-3 border-b border-gray-100 last:border-0">
      ...
      <div className="min-w-0">
        ...
        <p className={`text-sm font-medium break-all ${href ? 'text-blue-600' : 'text-gray-800'}`}>
          {value}
        </p>
      </div>
    </div>
...
function FieldIcon({ name }: { name: string }) {
  ...
  switch (name) {
    ...
    case 'map-pin':
      return <svg className={cls} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>;
    case 'linkedin':
      return <svg className={cls} viewBox="0 0 24 24" fill="currentColor"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" /><rect x="2" y="9" width="4" height="12" /><circle cx="4" cy="4" r="2" /></svg>;
    case 'twitter':
