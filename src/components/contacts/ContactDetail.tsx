...
export function ContactDetail({ contactId }: Props) {
  ...
  if (!contact) {
    return <div className="p-4 text-center text-gray-500 py-12">Contact not found.</div>;
  }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="p-4 space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {contact.tags.map((tag) => (
                <span key={tag} className="text-sm bg-blue-50 text-blue-700 px-3 py-1 rounded-full">{tag}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {showDeleteConfirm ? (
          ...
          <button onClick={() => setShowDeleteConfirm(true)} className="w-full py-3 text-red-600 font-medium text-sm border border-red-200 rounded-xl hover:bg-red-50 transition-colors">
            Delete Contact
          </button>
        )}
      </div>
