      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100">
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              Required for card scanning. Get one at{' '}
          ...
          <div className="p-4 space-y-3">
            ...
            {testResult === 'error' && (
              <p className="text-red-600 text-sm break-all">{testError}</p>
            )}
            <button
              onClick={handleTest}
              disabled={testing}
              className="flex items-center justify-center gap-2 w-full py-2.5 border border-blue-300 text-blue-600 rounded-lg text-sm font-medium hover:bg-blue-50 disabled:opacity-50 transition-colors"
            >
            ...
            {[
              ...
              <label key={m.id} className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50">
                <input
                  ...
                  name="model"
                  value={m.id}
                  checked={state.settings.model === m.id}
                  onChange={() => dispatch({ type: 'SET_SETTINGS', settings: { model: m.id as typeof state.settings.model } })}
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100">
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {state.contacts.length} card{state.contacts.length !== 1 ? 's' : ''} · ~{storageKB} KB used locally
            </p>
          </div>
          <div className="p-4 space-y-3">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-2.5 border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 disabled:opacity-40 transition-colors"
            >
        ...
        <div className="text-center text-xs text-gray-400 pb-2">
          Card Tracker · All data stored locally on your device
        </div>
      </div>
    </div>
  );
