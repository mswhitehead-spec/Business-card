...
export function Settings() {
  ...
  useEffect(() => { isModelCached().then(setVlmCached); }, []);

  function saveKey() { dispatch({ type: 'SET_SETTINGS', settings: { anthropicApiKey: apiKey.trim() } }); }
  ...
  async function handlePreload() {
    setPreloading(true); setPreloadMsg('Starting download...');
    try {
      await preloadModel(({ message }) => setPreloadMsg(message));
      setVlmCached(true); setPreloadMsg('');
    } catch (err) {
      ...
      <div className="p-4 space-y-4">
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100">
            ...
            <p className="text-xs text-gray-400 mt-0.5">Without a key, the app uses on-device AI (WebGPU) or basic OCR. Add a key for Claude AI extraction.{' '}<a href="https://console.anthropic.com" target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">console.anthropic.com</a></p>
          </div>
          <div className="p-4 space-y-3">
            ...
            {testResult === 'success' && (<p className="text-green-600 text-sm flex items-center gap-1.5"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} className="w-4 h-4"><polyline points="20 6 9 17 4 12" /></svg>API key is valid</p>)}
            {testResult === 'error' && <p className="text-red-600 text-sm break-all">{testError}</p>}
            <button onClick={handleTest} disabled={testing} className="flex items-center justify-center gap-2 w-full py-2.5 border border-blue-300 text-blue-600 rounded-lg text-sm font-medium hover:bg-blue-50 disabled:opacity-50 transition-colors">{testing ? <><Spinner size={16} /> Testing...</> : 'Test API Key'}</button>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100">
          ...
          <div className="divide-y divide-gray-100">
            {[
              ...
              <label key={m.id} className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50">
                ...
                <div><p className="text-sm font-medium text-gray-800">{m.label}</p><p className="text-xs text-gray-400">{m.desc}</p></div>
              </label>
            ))}
          </div>
        </div>
        {!state.settings.anthropicApiKey && webGPU && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-4 py-3 border-b border-gray-100">
              ...
              <p className="text-xs text-gray-400 mt-0.5">SmolVLM-256M runs locally in your browser. First use downloads ~200 MB (cached after that).</p>
            </div>
            <div className="p-4 space-y-3">
              {vlmCached === null ? (<p className="text-xs text-gray-400">Checking cache...</p>) : vlmCached ? (
                <div className="flex items-center gap-2"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} className="w-4 h-4 text-green-600 flex-shrink-0"><polyline points="20 6 9 17 4 12" /></svg><p className="text-sm text-green-700">Model is cached and ready</p></div>
              ) : (<p className="text-sm text-gray-500">Model not yet downloaded</p>)}
              {preloadMsg !== '' && <p className="text-xs text-blue-600">{preloadMsg}</p>}
              <button onClick={handlePreload} disabled={preloading || vlmCached === true} className="w-full flex items-center justify-center gap-2 py-2.5 border border-blue-300 text-blue-600 rounded-lg text-sm font-medium hover:bg-blue-50 disabled:opacity-50 transition-colors">
