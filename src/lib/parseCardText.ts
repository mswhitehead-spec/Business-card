...
export function parseCardText(text: string): Partial<BusinessCard> {
  ...
  const bareDomainMatch =
    cleaned.match(/(?<!\S)([a-z0-9\-]+\.(?:com|io|co|net|org|app|dev|ai))(?!\S)/i)?.[0] ?? '';
  const bareWebsite =
    bareDomainMatch && bareDomainMatch !== emailDomain ? `https://${bareDomainMatch}` : '';
  ...
  // Prefer lines that look like proper names; fall back to first line
  const nameLine = textLines.find(looksLikeName) ?? textLines[0] ?? '';
  const rest = textLines.filter((l) => l !== nameLine);

  // Prefer lines with job-title keywords for title; fall back to first remaining
  const titleLine = rest.find((l) => TITLE_WORDS.test(l)) ?? rest[0] ?? '';
  const rest2 = rest.filter((l) => l !== titleLine);

  // Prefer lines with company-suffix keywords for company
